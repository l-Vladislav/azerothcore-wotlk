...
### Chest opening flow
1. Player right-clicks chest scroll/item in inventory.
2. C++ `ItemScript::OnUse` resolves chest type → corresponding pool table.
3. Weighted RNG picks one scroll entry from the pool.
4. Scroll added to player inventory via `AddItemOrMail` (mail fallback if bag full).
5. Chest item consumed.
6. Chat-line announce: "Вам выпал [Имя питомца]!" — colour-coded by rarity (white/blue/purple).
