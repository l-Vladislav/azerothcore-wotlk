...
export class ListView<T> implements FilterState, SortState {
  ...
  // Десять строк по умолчанию: строка листа - это пять-десять полей и
  // списков, и полсотни строк давали десятки тысяч узлов на вкладку.
  readonly perPage = signal(10);
  /**
   * Строка, к которой привела ссылка со «Связей». Карта обязана приводить
   * именно К СТРОКЕ: страница основ - это восемь десятков записей на
