...
RULES: list[tuple[str, re.Pattern, str, str, str]] = [
    ...
    ("POST", re.compile(r"^/api/apply$"), "system",
     "", "правки применены на живом сервере"),
    # витрина лаунчера: пишет сборщик, но просит отсюда - и здесь же видно,
    # кто и когда менял то, что игроки прочтут при следующем запуске
    ("PUT", re.compile(r"^/api/cdn/news$"), "cdn",
     "news.json", "обновлены новости лаунчера"),
    ("PUT", re.compile(r"^/api/cdn/ui$"), "cdn",
     "ui.json", "обновлены подписи окна лаунчера"),
    # добыча: страница читает игровые таблицы и пишет только свои подписи
    ("PUT", re.compile(r"^/api/loot/label/(?P<table>\w+)/(?P<entry>\d+)$"),
     "loot", "{table} {entry}", "переименована запись или группа добычи"),
    ("PUT", re.compile(r"^/api/loot/row/(?P<table>[a-z_]+)/(?P<entry>\d+)$"),
     "loot", "{table} {entry}", "правка строки добычи"),
    ("DELETE", re.compile(r"^/api/loot/row/(?P<table>[a-z_]+)/(?P<entry>\d+)$"),
     "loot", "{table} {entry}", "удалена строка добычи"),
    ("POST", re.compile(r"^/api/loot/group/(?P<table>[a-z_]+)/(?P<entry>\d+)$"),
     "loot", "{table} {entry}", "строки перенесены в другую группу"),
    ("POST",
     re.compile(r"^/api/loot/group/(?P<table>[a-z_]+)/(?P<entry>\d+)/collect$"),
     "loot", "{table} {entry}", "строки собраны в группу"),
    ("POST", re.compile(r"^/api/loot/reload/(?P<table>[a-z_]+)$"),
     "loot", "{table}", "таблица добычи перечитана миром"),
    # weather
    ...
    ("POST", re.compile(r"^/api/weather/release$"), "weather",
     "все зоны", "ручная погода возвращена режиссёру"),
    ("PUT", re.compile(r"^/api/weather/zones/(?P<id>\d+)/links$"), "weather",
     "зона {id}", "изменены связи зоны"),
    ("PUT", re.compile(r"^/api/weather/zones/(?P<id>\d+)/climate$"), "weather",
     "зона {id}", "изменён климат зоны (ждёт перезапуска мира)"),
    ...
    ("POST", re.compile(r"^/api/weather/cyclones$"), "weather",
     "фронты", "заведён свой фронт"),
    ("POST", re.compile(r"^/api/weather/queue$"), "weather",
     "очередь", "добавлена заготовка фронта"),
    ("PUT", re.compile(r"^/api/weather/queue/(?P<id>\d+)$"), "weather",
     "очередь #{id}", "правка заготовки фронта"),
    ("DELETE", re.compile(r"^/api/weather/queue/(?P<id>\d+)$"), "weather",
     "очередь #{id}", "заготовка фронта убрана"),
    ("PUT", re.compile(r"^/api/weather/cyclones/(?P<id>\d+)$"), "weather",
     "фронт {id}", "правка своего фронта"),
    ("DELETE", re.compile(r"^/api/weather/cyclones/(?P<id>\d+)$"), "weather",
     "фронт {id}", "фронт убран"),
    ...
    ("PUT", re.compile(r"^/api/aprof/materials$"), "aprof",
     "материалы", "изменён материал"),
    ("DELETE", re.compile(r"^/api/aprof/materials/(?P<entry>\d+)$"), "aprof",
     "материал {entry}", "удалён материал"),
    ("PUT", re.compile(r"^/api/aprof/bases$"), "aprof",
     "основы", "изменена основа"),
    ("PUT", re.compile(r"^/api/aprof/types$"), "aprof",
     "типы", "изменён тип предмета"),
    ("DELETE", re.compile(r"^/api/aprof/types/(?P<id>\d+)$"), "aprof",
     "тип {id}", "удалён тип предмета"),
    ("PUT", re.compile(r"^/api/aprof/types/(?P<id>\d+)/parts$"), "aprof",
     "тип {id}", "изменена часть типа"),
    ("DELETE", re.compile(r"^/api/aprof/types/(?P<id>\d+)/parts/(?P<idx>\d+)$"),
     "aprof", "тип {id}, часть {idx}", "удалена часть типа"),
    ("PUT", re.compile(r"^/api/aprof/bases/(?P<id>\d+)/recipe$"), "aprof",
     "основа {id}", "изменён рецепт основы"),
    ("DELETE", re.compile(r"^/api/aprof/bases/(?P<id>\d+)$"), "aprof",
     "основа {id}", "удалена основа"),
    ("PUT", re.compile(r"^/api/aprof/synergies$"), "aprof",
     "синергии", "изменена синергия"),
    ("DELETE", re.compile(r"^/api/aprof/synergies/(?P<id>\d+)$"), "aprof",
     "синергия {id}", "удалена синергия"),
    ("PUT", re.compile(r"^/api/aprof/balance$"), "aprof",
     "баланс", "изменены качества и корзины навыка"),
    ("PUT", re.compile(r"^/api/aprof/settings$"), "aprof",
     "настройки", "изменены числа модуля"),
    ("POST", re.compile(r"^/api/aprof/reload$"), "aprof",
     "", "перечитаны справочники на PTR"),
    # spells
    ...
    # items
    ("PUT", re.compile(r"^/api/items/(?P<entry>\d+)$"), "items",
     "предмет {entry}", "сохранён предмет"),
    ("DELETE", re.compile(r"^/api/items/(?P<entry>\d+)$"), "items",
     "предмет {entry}", "удалён предмет"),
    ("POST", re.compile(r"^/api/items/export/sql$"), "items",
     "", "предметы выгружены в SQL"),
    ("POST", re.compile(r"^/api/items/export/client$"), "items",
     "", "предметы выгружены в клиентский CSV"),
    # board
    ...
    # клиентский патч
    ("POST", re.compile(r"^/api/patch/build$"), "patch",
     "", "запущена сборка клиентского патча"),
    ("POST", re.compile(r"^/api/patch/bootstrap$"), "patch",
     "", "база DBC снята заново с архивов CDN"),
]
