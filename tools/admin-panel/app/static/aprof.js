...
function itemCell(item, fallback) {
  ...
  if (item) {
    ...
    // Предмета в item_template нет: основа будет ссылаться в пустоту, и в игре
...
// --- основы ---------------------------------------------------------------
// Разворот 2026-09-02: основа - это точный набор предметов по ячейкам и
...
// подсказку кнопки: основа с дырой выглядит рабочей, а в игре это «кнопка
// молча не работает».
...
function renderRecipes(view) {
  ...
  // Полоса добавления заводит основу ЦЕЛИКОМ: строку основы, изделие на
  ...
  // Книга задаётся сразу, а не отдельным заходом в строку таблицы: основу без
  // неё можно только угадать, и «завести основу» без этого выбора - половина
  // дела. В памяти полосы книга НЕ живёт: одна книга учит одной основе
  ...
  table.appendChild(headRow('recipes', heads, spec, (th, h) => {
    ...
    if (h === 'Обучение') {
      th.title = 'Обучающий предмет. Пусто — основу можно только угадать.';
    }
    if (h === 'Откуда') {
  ...
  rows.forEach(rec => {
    ...
    // открывает вкладку с восемью десятками основ.
    ...
    // Набор. Сколько ячеек схемы заполнено — по этому числу видно, дописана ли
    // основа: пустые ячейки в точном сравнении значат «пусто», а не «любой».
    ...
    tr.appendChild(el('td')).appendChild(
      select(Object.entries(ACQUIRE), rec.acquire || 'forge', v => patch({
        ...
        // не примет, а оставленная книга обещала бы основу, которую не сковать.
    ...
    if (drop) {
      ...
      teachTd.title = 'Книга обещала бы основу, которую не сковать. Именные '
    ...
    resBtn.title = 'Изделия основы: по строке item_template на ступень.';
...
// Завести основу целиком: строка основы, изделие на каждую ступень диапазона
...
async function removeRecipe(rec) {
  if (!confirm(`Удалить основу «${rec.name_ru}»? Заведённые изделия `
...
// основы, и живут они там же, где урон и качество. Класс с подклассом пока
...
function renderTypes(view) {
  ...
  table.appendChild(headRow('types',
    ['Имя', 'Подпись', 'Код', 'Класс', 'Подкласс', 'Внешний вид', 'Чертёж',
     'Поделка', 'Порядок', 'Вкл', 'Частей', 'Основ', ''], spec, (th, h) => {
      if (h === 'Чертёж') {
        th.title = 'Имя файла из textures аддона, без пути и расширения: '
          + 'item-sword. Пусто — аддон возьмёт своё умолчание по классу.';
      }
      if (h === 'Поделка') {
        th.title = 'Серая вещь, которую выдаёт набор, не совпавший ни с одной основой. Нужна, пока '
          + 'fail_mode = 2.';
      }
    }));
  ...
  rows.forEach(t => {
    ...
    // правится в каталоге - как и изделия основ.
...
async function openPartsEditor(itemType) {
  ...
  async function remove(idx) {
    if (!confirm('Убрать часть ' + idx + '? Уже собранные предметы держат ' +
                 ...
                 'основ на эту часть будут удалены.')) return;
    try {
  ...
  async function draw() {
    ...
    // убраны миграцией v15 - числа изделия приходят из результата основы,
    // расход задаёт основа, геометрия живёт в аддоне, а пустая ячейка стала
    ...
    rows.forEach(part => {
      ...
      // пуста, а основу, которая её не называет, панель не даст включить.
      const needTd = el('td');
      const need = el('input');
      need.type = 'checkbox';
      need.checked = !!part.required;
      need.title = 'Без этой части предмет не собрать: пустой её не оставить, '
        + 'и каждая основа этого типа обязана её называть.';
...
// подробнее основа, тем она сильнее, поэтому частный случай не обязан
// перечислять весь набор.

// --- выбор предмета -------------------------------------------------------
// Основе нужны чужие предметы: обучающий свиток, образец для изделия, готовое
  ...
  async function draw() {
    ...
    data.rows.forEach(row => {
      ...
      sel.title = row.required
        ? 'Ячейка обязательная: пока материал не выбран, основу нельзя '
          + 'включить — сковать её не выйдет.'
        : 'Пусто — основа требует, чтобы игрок оставил эту ячейку '
...
// каждую основу. Отсюда кнопка «создать варианты» — копия образца на каждую
...
async function openFiltersEditor(rec) {
  ...
  function draw() {
    ...
    // я до пустоты»: ноль камней при живых гнёздах основу включить не даст.
...
async function openResultsEditor(rec) {
  ...
  async function draw() {
    ...
    if (!fresh) {
      body.appendChild(el('div', 'empty', 'Основа пропала.'));
      return;
    }
...
// (качество, границы уровня и фильтры основы - те же четыре условия, что
...
async function renderBalance(view) {
  ...
  const chHead = el('tr');
  ['Разрыв навык − req_skill основы', 'Шанс успеха'].forEach(
...
function renderPreview(view) {
  ...
  // основами ТОЧНО (предмет и количество), а не «считается предмет». Числа
  ...
  async function drawParts() {
    ...
    rows.forEach(part => {
      ...
      // Количество - часть набора: «те же предметы, но по три» это другая
      // основа (DESIGN §5.4.2), поэтому его задают здесь, а не берут из схемы.
    ...
    try {
      ...
      if (!res.recipe) {
        ...
        out.appendChild(el('div', null,
          `Совпала основа «${r.name_ru}» (id ${r.id}), требует навыка ${r.req_skill}`));
        const c = res.chance;
...
async function renderPool(view) {
  ...
  // Основы без слайса. Опаснее пустого списка: основа выглядит готовой, ковка
  // по ней идёт, а доводка отвечает «кончились свободные id» - на вид
  ...
  const table = el('table', 'grid');
  const head = el('tr');
  ['id', 'Изделие', 'Основа', 'Вставки', 'Когда']
...
// «Основы» и «Именные» это собиралось в голове, сверкой номеров.
//
...
// карта приводила бы к таблице из восьми десятков основ, и дальше пришлось
// бы искать глазами.
...
function mapRecipe(rec, syns) {
  ...
  head.appendChild(el('b', null, rec.name_ru || ('основа ' + rec.id)));
...
function graphLayout(state) {
  ...
  // Полоса. Своего поля у основы нет - лестница живёт в ap_band, а связь с
  // ней одна: требуемый навык. Отбор поэтому идёт по `skill` полосы, а не по
  // её номеру: номер у основы взять неоткуда.
...
function renderGraph(view, mem) {
  ...
  // Полосы. Ключ чипа - НАВЫК полосы, а не её номер: именно им основа с ней и
  // связана. Базы без v32 полос не отдают вовсе - тогда строки просто нет.
